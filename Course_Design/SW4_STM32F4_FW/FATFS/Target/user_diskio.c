...
unsigned char *binDisplay(char *pic_name)
{
    f_res = f_open(&file, pic_name, FA_READ);
    f_res = f_read(&file, ReadBuffer, 40960, &bw);
    f_res = f_close(&file);
    return ReadBuffer;
}
