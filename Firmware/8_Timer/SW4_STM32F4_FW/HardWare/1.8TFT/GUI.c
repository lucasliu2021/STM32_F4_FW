...
void LCD_DrawWave(uint16_t wave[], uint8_t drawline, uint16_t colm, uint16_t heigh, uint16_t color)
{
    ...
    for (; lie_size > 0; lie_size--)
    {
        ...
        Gui_DrawPoint(lie_size, colm - (unsigned char) (wave[i] * heigh / 4095), color);

        if ((i > 1) && (i < 128) && drawline)
        {
            if ((wave[i] - wave[i - 1] > 74 || (wave[i] - wave[i - 1] < -74)))//4096/HIGH=74

                Gui_DrawLine(lie_size, colm - (unsigned char) (wave[i] * heigh / 4095), lie_size + 1,
                             colm - (unsigned char) (wave[i - 1] * heigh / 4095), color);
        }
